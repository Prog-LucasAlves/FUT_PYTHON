...
def get_last_10_team_summary(df_games, team_name, target_score):
    ...
    return {
        ...
        "ht_02": build_ht_scenario_summary(team_games, (0, 2)),
    }
...
def calculate_pro_metrics(df_games, home_team, away_team, current_match_data):
    ...
    poisson_0x2 = (prob_h0 * prob_a2) * 100
    ...
    def analyze_ht_scenarios(games_h, games_a):
        ...
        prob_red_from_00 = (
            (
                (
                    len(
                        h_00_ht[(h_00_ht["Goals_H_FT"] == 0) & (h_00_ht["Goals_A_FT"] == 2)],
                    )
                    + len(
                        a_00_ht[(a_00_ht["Goals_H_FT"] == 0) & (a_00_ht["Goals_A_FT"] == 2)],
                    )
                )
                / total_00_ht
                * 100
        ...
        prob_red_from_01 = (
            (
                (
                    len(
                        h_01_ht[(h_01_ht["Goals_H_FT"] == 0) & (h_01_ht["Goals_A_FT"] == 2)],
                    )
                    + len(
                        a_01_ht[(a_01_ht["Goals_H_FT"] == 0) & (a_01_ht["Goals_A_FT"] == 2)],
                    )
                )
                / total_01_ht
                * 100
    ...
    odd_lay_0x2 = current_match_data.get("Odd_CS_0x2_Lay", 0)
    odd_btts = current_match_data.get("Odd_BTTS_Yes_Back", 0)
    odd_over25 = current_match_data.get("Odd_Over25_FT_Back", 0)

    # 1º Condição 1.50-1.79 | 4.00-4.99 | 20.0+
    cond1 = (1.50 <= odd_h_back <= 1.79) and (4.00 <= odd_a_back <= 4.99) and (odd_lay_0x2 >= 20.00)
    # 2º Condição 3.50-3.99 | 2.10-2.49 | 15.0-15.9
    cond2 = (3.50 <= odd_h_back <= 3.99) and (2.10 <= odd_a_back <= 2.49) and (15.0 <= odd_lay_0x2 <= 15.9)
    # 3º Condição 4.00-4.99 | 1.80-2.09 | 10.0-10.9
    cond3 = (4.00 <= odd_h_back <= 4.99) and (1.80 <= odd_a_back <= 2.09) and (10.0 <= odd_lay_0x2 <= 10.9)
    # 4º Condição 2.10-2.49 | 2.50-2.99 | 20.0+
    cond4 = (2.10 <= odd_h_back <= 2.49) and (2.50 <= odd_a_back <= 2.99) and (odd_lay_0x2 >= 20.00)
    # 5º Condição 3.00-3.49 | 2.50-2.99 | 16.0-17.9
    cond5 = (3.00 <= odd_h_back <= 3.49) and (2.50 <= odd_a_back <= 2.99) and (16.0 <= odd_lay_0x2 <= 17.9)
    # 6º Condição 3.00-3.49 | 2.10-2.49 | 15.0-15.9
    cond6 = (3.00 <= odd_h_back <= 3.49) and (2.10 <= odd_a_back <= 2.49) and (15.0 <= odd_lay_0x2 <= 15.9)
    # 7º Condição 4.00-4.99 | 1.80-2.09 | 14.0-14.9
    cond7 = (4.00 <= odd_h_back <= 4.99) and (1.80 <= odd_a_back <= 2.09) and (14.0 <= odd_lay_0x2 <= 14.9)
    # 8º Condição 5.00+ | 1.80-2.09 | 9.00-9.90
    cond8 = (odd_h_back >= 5.00) and (1.80 <= odd_a_back <= 2.09) and (9.00 <= odd_lay_0x2 <= 9.90)
    # 9º Condição 3.00-3.49 | 2.10-2.49 | 20.0+
    cond9 = (3.00 <= odd_h_back <= 3.49) and (2.10 <= odd_a_back <= 2.49) and (odd_lay_0x2 >= 20.00)
    # 10º Condição 3.50-3.99 | 1.80-2.09 | 15.0-15.9
    cond10 = (3.50 <= odd_h_back <= 3.99) and (1.80 <= odd_a_back <= 2.09) and (15.0 <= odd_lay_0x2 <= 15.9)

    if any([cond1, cond2, cond3, cond4, cond5, cond6, cond7, cond8, cond9, cond10]):
        score += 5
        reasons.append("Padrão de Odds Detectado (Match Odds + Lay 0x2)")
    ...
    if poisson_0x2 < 7:
        score += 2
        reasons.append(f"Baixa probabilidade Poisson ({poisson_0x2:.1f}%)")

    combined_success = 100 - poisson_0x2
    if combined_success > 92:
        score += 2
        reasons.append(f"Sucesso histórico excelente ({combined_success:.1f}%)")

    # Cálculo de CLV (Closing Line Value)
    odd_open = current_match_data.get("Odd_CS_0x2_Lay", 0)
...
# Interface
st.title("🛡️ Lay 0x2 Ultimate - Professional Trading Tool")
main_tab, bets_tab = st.tabs(["🛡️ Lay 0x2 Ultimate", "🧾 Planilha"])
    ...
    if not df_today.empty:
        ...
        if not df_day_filtered.empty:
            ...
            if results:
                ...
                with st.form(key=f"bet_form_lay02_{m_data['Home']}_{m_data['Away']}"):
                    f1, f2, f3 = st.columns(3)
                    with f1:
                        odd_entrada = st.number_input(
                            "Odd Entrada",
                            min_value=1.01,
                            value=float(m_data.get("Odd_CS_0x2_Lay", 0) or 1.01),
                            step=0.01,
                        )
                    with f2:
            ...
            with o4:
                st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                st.write("**Correct Score 0x2**")
                odd_cs_back = m_data.get("Odd_CS_0x2_Back", 0)
                odd_cs_lay = m_data.get("Odd_CS_0x2_Lay", 0)
                if odd_cs_back > 0:
            ...
            with o5:
                ...
                odd_lay = m_data.get("Odd_CS_0x2_Lay", 0)
                if odd_lay > 0:
            ...
            st.markdown("---")
            if m_data.get("Odd_CS_0x2_Lay", 0) == 0:
                st.warning("⚠️ **Atenção:** Odds de Correct Score 0x2 não encontradas para este jogo no arquivo de hoje. A análise de Score e Valor (EV/CLV) está limitada.")

            st.subheader("🎯 Recomendação de Estratégia Lay 0x2")
            ...
            with ex1:
                ...
                st.write("**1. Gol do Mandante (1-0):** O jogo 'morreu' para o Lay 0x2. Você pode fechar com lucro total ou deixar rolar (se for Lay puro).")
                st.write("**2. Terceiro Gol do Visitante (0-3) ou Primeiro do Mandante (1-2):** Placar de 0x2 impossível. Lucro garantido.")
                st.write("**3. Empate (0-0, 1-1, 2-2):** Placar de 0x2 impossível. Lucro garantido.")
                st.write("**4. Final do Jogo:** Se o jogo terminar com qualquer resultado que não seja 0-2, a aposta é vencedora no Lay 0x2.")
            ...
            with ex2:
                ...
                st.write("- **DECISÃO:** MONITORAR. Se o jogo estiver muito aberto, considere sair.")
                st.write("- **POR QUE?** O próximo gol do visitante causa o RED total (0x2).")
                st.write("**Placar 0x2 no HT:**")
                st.write("- **DECISÃO:** RED TOTAL. O jogo atingiu o placar de lay pre-match.")

            with ex3:
                st.error("🛑 **SAÍDA POR TEMPO (LIMIT EXPOSURE)**")
                st.write(f"**Minuto Limite: {exit_minute}' a 80'**")
                st.write(f"- Se o placar persistir em **0x0** ou **0x1** até o minuto **{exit_minute}'**, realizar o CASH OUT (Hedge).")
                st.write("- O risco de dois gols ou de um gol fatal (0x2) nos acréscimos é o cenário de maior prejuízo.")
                st.write("- **Stop Loss Fixo:** Se o 0x2 acontecer, o red é total. Se o 0x1 acontecer após os 70', saia imediatamente se o mandante estiver apático.")

            st.info(f"""
            💡 **Estratégia Vencedora (Luke 3.0):**
            O segredo do Lay 0x2 não é apenas acertar o jogo, mas saber sair quando o cenário muda.
            ...
            with c1:
                st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                st.metric("Prob. Poisson 0x2", f"{results['poisson_0x2']:.2f}%")
                st.write(
            ...
            with p1:
                ...
                st.write(f"0x2 HT: {last10['ht_02']['total']} | Gol mandante até 75': {last10['ht_02']['home_goal_to_75']} | Gol visitante até 75': {last10['ht_02']['away_goal_to_75']}")
                st.write(f"Permaneceu 0x2 até 75': {last10['ht_02']['stayed_score_to_75']} | Outro placar: {last10['ht_02']['changed_score_to_75']}")
                st.markdown("</div>", unsafe_allow_html=True)
            with p2:
            ...
            scenario_02_home = build_poisson_timing_scenario(df_hist, m_data["Home"], "home", (0, 2))
            scenario_02_away = build_poisson_timing_scenario(df_hist, m_data["Away"], "away", (0, 2))
            ...
            render_poisson_time_block("Cenário 0x2 aos 75'", scenario_02_home, scenario_02_away)
    ...
    if df_bets_all.empty:
        ...
        with e1:
            saida_edit = st.selectbox(
                ...
                key=f"saida_edit_lay02_{selected_bet_idx}",
            )
        with e2:
            ...
            odd_saida_edit = st.number_input(
                ...
                key=f"odd_saida_edit_lay02_{selected_bet_idx}",
            )
        with e3:
        ...
        a1, a2 = st.columns(2)
        with a1:
            if st.button("Salvar Saída", use_container_width=True, key=f"save_exit_lay02_{selected_bet_idx}"):
        ...
        with a2:
            if st.button("Excluir Entrada", use_container_width=True, key=f"delete_bet_lay02_{selected_bet_idx}"):
        ...
        with s4:
            ...
            st.write("Mercado dominante: Lay 0x2")
            st.markdown("</div>", unsafe_allow_html=True)
